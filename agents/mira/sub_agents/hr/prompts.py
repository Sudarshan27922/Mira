    ...
    return ChatPromptTemplate.from_messages([
        (
            ...
            (
                ...
                "     - Use query_user_calendar to check a user's calendar\n"
                "     - ALWAYS extract 'Today's date: YYYY-MM-DD' from the beginning of the prompt\n"
                "     - Calculate absolute dates from relative ones using today's date:\n"
                "       * 'tomorrow' -> YYYY-MM-DD (today + 1 day)\n"
                "       * 'Monday'/'Tuesday'/etc -> next occurrence as YYYY-MM-DD\n"
                "       * 'next week' -> Monday to Friday as two dates\n"
                "     - ALWAYS call query_user_calendar with actual YYYY-MM-DD dates, NOT relative strings\n"
                "     - For single day queries (tomorrow, Monday), use same date for start_date and end_date\n"
                "     - Example: If today is 2025-10-26 and user says 'tomorrow', call with start_date='2025-10-27', end_date='2025-10-27'\n"
                "     - Example: If today is 2025-10-24 (Thursday) and user says 'Monday', calculate next Monday (2025-10-27) and use that\n"
                "     - Pass user_email from user_context['emp_email']\n"
                "     - Present the tool results to the user in a friendly, readable format\n"
