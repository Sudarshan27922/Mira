    ...
    return ChatPromptTemplate.from_messages([
        (
            ...
            (
                ...
                "  3. If user context has emp_email and manager_email, and space_name is found in prompt:\n"
                "     - IMMEDIATELY call leave_process_workflow with:\n"
                "       * payload: {{'employee_email': user_context['emp_email'], 'supervisor_email': user_context['manager_email']}}\n"
                "       * space_name: extracted from prompt (look for 'Space:' or 'spaces/')\n"
                "  4. The workflow will automatically send an interactive card for collecting leave details\n"
                "  5. ONLY ask for missing information if user context is incomplete or space_name is not found\n"
                "  6. CRITICAL: If user context has emp_email and manager_email, DO NOT ask for them again\n"
                "  7. CRITICAL: If space_name is in the prompt, DO NOT ask for it again - it's already provided\n"
                "  8. CRITICAL: The space name comes from the incoming message and is automatically included in the prompt\n"
                "- If the user provides complete leave data (employee_email, leave_type, start_date, end_date, reason, supervisor_email), "
                "  call leave_process_workflow with the complete payload to process the request.\n"
