    ...
    return ChatPromptTemplate.from_messages([
        (
            ...
            (
                ...
                "Routing rules:\n"
                "- If the user requests an end-to-end leave application:\n"
                "  1. Extract employee_email from user context (required)\n"
                "  2. Extract supervisor_email (reporting_manager) from user context (required)\n"
                "  3. Extract space_name from the prompt (format: 'Space: <space_name>')\n"
                "  4. If all three are available, call leave_process_workflow with employee_email, supervisor_email, and space_name\n"
                "  5. The workflow will automatically send an interactive card for collecting leave details\n"
                "  6. If user context is missing employee_email or reporting_manager, ask for clarification\n"
